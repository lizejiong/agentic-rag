...
- 用途：产品验收与故障定位；20 份完全虚构的中文多格式文件、25 道固定问题，全部位于“星桥云途验收空间”。
- 指标：Vector、ES、RRF、Reranker 的 `Hit@1`、`Hit@5`、`Hit@10`、MRR 和耗时；完整验收还记录答案要点覆盖率、引用证据正确率和拒答正确率。
- 边界：D003 用于中文企业资料场景，不与英文 SciFact 的分数直接比较。
- 管理方式：评测数据集、问题和运行记录由评测中心保存到数据库；测试文件仍使用绑定知识空间的正常文档上传流程。
