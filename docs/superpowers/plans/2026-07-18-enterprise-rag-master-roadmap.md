...
| 顺序 | 子计划 | 可独立交付的软件增量 | 前置 | 状态 |
|---:|---|---|---|---|
| 0 | 仓库骨架与流协议 | React → NestJS → Python 的可取消假流式问答，CI 通过 | 无 | ✅ |
| 1 | 本地数据平台与身份权限 | Docker Compose 数据层、本地账号、两角色、空间 ACL、审计 | 0 | ✅ |
| 2 | 文档导入与生命周期 | 全格式上传、URL 单页、OCR/解析适配、版本、任务、删除恢复 | 1 | 🟡 收尾 |
| 3 | 混合检索与可验证问答 | pgvector + BM25 + RRF + Reranker、SSE 引用、反馈、短期记忆 | 2 | 🟡 11/12 |
...
**详细计划：** `docs/superpowers/plans/2026-07-21-document-ingestion-foundation.md`、`docs/superpowers/plans/2026-07-22-secure-url-import.md`

**范围：**

- PDF、DOCX、DOC、XLSX、XLS、PPTX、PPT、TXT、MD、CSV、JSON 共 11 种格式。
- 单页 URL 安全抓取与 SSRF 防护。
- 文件签名、MIME、病毒、压缩炸弹和解析资源限制。
- LibreOffice 隔离转换、扫描 PDF OCR、Office 关键图片 OCR（后续硬化增量）。
...
**状态：** 🟡 核心交付完成，收尾任务见 [Phase 2 收尾计划](2026-07-25-phase2-closeout.md)。
...
- 文档删除后 `rag.chunks` 和 `rag.chunk_embeddings` 同步清理。

## 6. 计划 3：混合检索与可验证问答

**详细计划：** `docs/superpowers/plans/2026-07-25-hybrid-retrieval-and-verifiable-qa.md`
...
**状态：** 🟡 11/12 任务完成（代码审计确认）。仅 mock 模型运行，真实模型接入属计划 5。
...
> 2026-07-25 代码审计后更新。已纠正此前基于推测的错误判断。

### 1. 知识库管理

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 创建、编辑、删除知识库 | 计划 1 | ✅ | Spaces CRUD |
| 不同知识库之间数据隔离 | 计划 1 | ✅ | ACL + SpacePolicy |
| 查看知识库文档数量和处理状态 | 计划 2 收尾 | 🟡 | SpacePolicy.list 加 `_count` 即可 |

### 2. 文档上传

| 功能 | 归属 | 状态 |
|------|------|------|
| 单文件/批量上传（最多 100 个，并发 3） | 计划 2 | ✅ |
| 支持 PDF、Word、TXT、Markdown 等 11 种格式 | 计划 2 | ✅ |
| 文件大小和格式校验 | 计划 2 | ✅ |
| 重复文件检测（content hash） | 计划 2 | ✅ |

### 3. 文档列表

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 文档名称、文件类型、大小、时间、处理状态 | 计划 2 | ✅ | |
| 上传人显示 | 计划 2 收尾 | 🟡 | `createdById` 被测试刻意排除（安全设计），需仅暴露 `createdBy.username` |
| 按名称搜索、按状态筛选 | 计划 2 收尾 | 🟡 | list 无 query 参数；`POST /search` 已有关键词搜索但仅限标题匹配 |

### 4. 文档处理状态

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 状态机：等待→安全检测→解析→标准化→切分→就绪/失败/取消 | 计划 2 | ✅ | |
| 处理失败时显示错误原因 | 计划 2 | ✅ | 详情页已展示 |
| 失败后手动重试 | 计划 2 收尾 | 🟡 | 自动恢复已实现（`DocumentReconciliationService`），缺用户触发端点 |

### 5. 文档解析与切分

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 自动管道：上传→提取→切分→生成向量→写入 pgvector + ES | 计划 2+3 | ✅ | Phase 3 已将 embedding + ES 索引接入 ingestion pipeline |
| Chunk 大小和重叠长度 UI 设置 | 计划 3 | 🟡 | Python settings 已有配置项，缺 UI |
| 切分结果预览 | 计划 3 | 🟡 | 缺 chunk list 端点 |
| 查看文档 Chunk 数量 | 计划 3 | 🟡 | `documentIngestionCompletedPayloadSchema` 已含 `chunkCount`，未暴露 API |

### 6. 文档详情

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 原始文件信息 + 解析文本 + 原文件下载 | 计划 2 | ✅ | |
| 版本历史 | 计划 2 | ✅ | |
| Chunk 列表 + 索引状态 | 计划 3 | 🟡 | `rag.chunks` 数据存在，缺 API 端点 |
| 失败原因 | 计划 2 | ✅ | |
| 重新解析/重新索引按钮 | 计划 3 | 🟡 | `begin()` 阻止已终态 run 重处理，需要 force re-index 机制 |

### 7. 文档更新与删除

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 删除文档 | 计划 2 收尾 | 🟡 | **服务层完整**（`setDocumentAvailability`），**缺 Controller 端点** |
| 删除时级联清理 Chunk 和向量 | 计划 2 收尾 | 🟡 | 完全未实现，需 NestJS 触发 rag schema 清理 |
| 替换已有文档 | 计划 2 收尾 | 🟡 | URL 版已实现（`refreshUrl`），FILE 版缺 |
| 删除后旧内容不可被检索 | 计划 3 | ✅ | Python 检索已做 `is_searchable` 标记 + ACL 过滤 |

### 8. 检索测试

| 功能 | 归属 | 状态 | 备注 |
|------|------|------|------|
| 输入问题，查看召回的文档和文本块 | 计划 3 | 🟡 | 检索管线已完整，缺独立测试端点 |
| 每个结果的相似度分数 | 计划 3 | 🟡 | `RetrievalSummary` 含 `rrfScore`、`rerankScore`，仅流式输出 |
| 文档名称和页码 | 计划 3 | ✅ | `CitationLocation` 含 page/slide/sheet |
| 最终提交给大模型的上下文 | 计划 3 | 🟡 | Agent 内部拼接，未暴露为 API |
