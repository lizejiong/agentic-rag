...
**现状：** `AuthorizationService.setDocumentAvailability()` 已实现完整逻辑（权限校验、审计日志、revision 更新），但无 HTTP 端点暴露。文档软删除后不会清理 `rag` 侧的 chunks、embeddings、normalized elements。

**步骤：**

- [ ] **步骤 1：新增 `DELETE /documents/:documentId` 端点**
...
**现状：** `DocumentsService.list()` 仅接受 `spaceId`，无搜索/筛选参数。`POST /search` 已有简单关键词匹配但仅限文档标题。

**步骤：**

- [ ] **步骤 1：list() 加 query 参数**
  - `search?: string`（标题模糊匹配）
  - `status?: string`（processingStatus 筛选）
  - 使用 Prisma `contains` + `where` 条件

- [ ] **步骤 2：Controller 接收 query params**
...
**现状：** `rag.chunks` 表有数据，无读取端点。`GET /documents/:documentId/content` 返回的是 normalized elements 拼接文本，不是 chunks。

**步骤：**

- [ ] **步骤 1：新增 `GET /documents/:documentId/chunks` 端点**
...
**现状：** `refreshUrl()` 已为 URL 类型实现了「新建版本 + 新 importTask」模式。FILE 类型无对应实现，`createFileImports()` 总是创建新 Document。

**步骤：**

- [ ] **步骤 1：新增 `POST /documents/:documentId/replace-file` 端点**
...
**现状：** `DocumentReconciliationService` 自动恢复 stalled 任务，但不处理 FAILED 终态。无用户触发重试的端点。

**步骤：**

- [ ] **步骤 1：新增 `POST /imports/:importId/retry` 端点**
  - 验证任务状态为 FAILED
...
- [ ] **步骤 1：`SpacePolicy.listVisible()` 加 Prisma `_count`**
...
**现状：** `createdById` 在现有测试中被刻意排除（`documents.service.spec.ts:136`），出于隐私设计。添加上传人需仅暴露 `createdBy.username`。

**步骤：**

- [ ] **步骤 1：list() 和 get() select 中 include `createdBy: { select: { username: true } }`**
...
- [ ] 文档可被软删除，删除后所有 API 返回 404，Chunk/向量同步清理
- [ ] 文档列表支持按标题搜索和按状态筛选
- [ ] 文档详情页展示 Chunk 列表（序号、文本、token 数、位置）
- [ ] FILE 类型文档可替换上传新版本
- [ ] 失败文档可通过按钮手动重试
- [ ] 知识空间列表展示文档总数
- [ ] 文档卡片显示上传人用户名
