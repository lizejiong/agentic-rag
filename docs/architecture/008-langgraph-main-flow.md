...
关系类问题在 `retrieve` 节点同时执行文档混合检索和图谱查询；其他问题只执行文档检索。查询改写最多一次，图配置 `recursion_limit=12`，因此不会形成无界循环。
...
节点使用 LangGraph `custom` stream 发出内部 `status`、`retrieval`、`token`、`citation` 事件；`Agent` runner 是唯一的 NDJSON 映射器和 sequence 分配者。每次检索摘要都包含 `attempt` 与实际生效查询。
