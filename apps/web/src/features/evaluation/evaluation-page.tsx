...
type Stage = 'vector' | 'lexical' | 'rrf' | 'rerank' | 'evidence';
...
const stageLabels: Record<Stage, string> = {
  ...
  rrf: '融合排序（RRF）',
  rerank: '重排序（Reranker）',
  evidence: '证据选择（Selector）',
};
const stageOrder: Stage[] = ['vector', 'lexical', 'rrf', 'rerank', 'evidence'];
const percentage = (value: number | null | undefined) => value == null ? '—' : `${Math.round(value * 100)}%`;
const compactStageLabels: Record<Stage, string> = { vector: '向量', lexical: 'ES', rrf: '融合', rerank: '重排', evidence: '证据' };
...
function EvaluationReportDrawer({
  ...
  const isAttentionRequired = (result: EvaluationResult) => Boolean(result.error)
    || !(result.stages.evidence?.hitAt10 ?? result.stages.rerank.hitAt10)
