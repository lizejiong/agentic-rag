...
describe('ActiveRunRegistry', () => {
  it('rejects a duplicate active request with a conflict', () => {
    ...
    expect(() => registry.start('request-1')).toThrow(ConflictException);
    expect(() => registry.start('request-1')).toThrow('REQUEST_ALREADY_RUNNING');
  });
  ...
  it('allows idempotent abort and restart after finish', () => {
    ...
    expect(() => registry.start('request-1')).not.toThrow();
  });
  ...
  it('does not let a stale finish remove a replacement run', () => {
    ...
    const replacement = registry.start('request-1');
