...
const conversation = { id: CONVERSATION_ID, title: '采购制度', createdAt: new Date('2026-08-12T00:00:00Z'), updatedAt: new Date('2026-08-12T00:00:00Z') };
...
describe('ConversationController', () => {
  ...
  let service: { listConversations: jest.Mock; createConversation: jest.Mock; getConversation: jest.Mock; renameConversation: jest.Mock; archiveConversation: jest.Mock };
  ...
  it('lists only the current user\'s active conversations', async () => {
    const response = await request(app.getHttpServer() as Server).get('/chat/conversations').expect(200);
    expect(response.body.conversations).toHaveLength(1);
    expect(response.body.conversations[0]).toEqual(expect.objectContaining({ id: CONVERSATION_ID, title: '采购制度' }));
  ...
  it('creates a conversation and validates titles before update', async () => {
    await request(app.getHttpServer() as Server).post('/chat/conversations').expect(201);
    await request(app.getHttpServer() as Server).patch(`/chat/conversations/${CONVERSATION_ID}`).send({ title: '   ' }).expect(400);
    await request(app.getHttpServer() as Server).patch(`/chat/conversations/${CONVERSATION_ID}`).send({ title: ' 新标题 ' }).expect(200);
  ...
  it('returns the detail and archives only the owner\'s conversation', async () => {
    await request(app.getHttpServer() as Server).get(`/chat/conversations/${CONVERSATION_ID}`).expect(200);
    await request(app.getHttpServer() as Server).delete(`/chat/conversations/${CONVERSATION_ID}`).expect(204);
