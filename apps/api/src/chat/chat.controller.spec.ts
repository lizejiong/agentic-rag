...
const testAuthGuard = {
  canActivate(context: ExecutionContext): boolean {
    context.switchToHttp().getRequest<AuthenticatedRequest>().user = {
      id: USER_ID, username: 'tester', role: 'MEMBER', tokenVersion: 0,
    };
    return true;
  },
};
...
class FakeAiEventSource implements AiEventSource {
  ...
  async *run(runRequest: RunRequestInput): AsyncIterable<AgentEvent> {
    ...
    const base = { requestId: runRequest.requestId, traceId: runRequest.traceId, occurredAt: '2026-07-18T00:00:00.000Z' } as const;
    yield { ...base, seq: 0, type: 'run.started' };
    yield { ...base, seq: 1, type: 'text.delta', text: 'server answer' };
    yield { ...base, seq: 2, type: 'citation', citationId: '00000000-0000-4000-8000-000000000011', chunkId: '00000000-0000-4000-8000-000000000012', documentId: '00000000-0000-4000-8000-000000000013', title: 'document', snippet: 'evidence', location: { page: 1 } };
...
describe('ChatController', () => {
  ...
  let conversations: {
    startTurn: jest.Mock; historyForRun: jest.Mock; completeTurn: jest.Mock;
    failTurn: jest.Mock; getOwnedTurn: jest.Mock;
  };
  ...
  beforeEach(async () => {
    ...
    conversations = {
      ...
      historyForRun: jest.fn().mockResolvedValue([{ role: 'user', content: 'trusted question' }, { role: 'assistant', content: 'trusted answer' }]),
    ...
    const module = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: AuthorizationService, useValue: { requireSpace, snapshot: jest.fn().mockResolvedValue({ userId: USER_ID, admin: false, groupIds: [], spaces: { [SPACE_ID]: 'VIEW' } }) } },
      ],
    }).overrideGuard(AccessTokenGuard).useValue(testAuthGuard).compile();
  ...
  it('loads canonical history and persists only observed AI events', async () => {
    const response = await request(app.getHttpServer() as Server)
      .post('/chat/stream').set('x-chat-protocol-version', '1').set('x-trace-id', 'trace-fixed')
      .send({ conversationId: CONVERSATION_ID, requestId: REQUEST_ID, selectedSpaceIds: [SPACE_ID], message: 'current question' })
      .expect(200);
    ...
    expect(fake.lastRequest).toEqual(expect.objectContaining({ requestId: REQUEST_ID, traceId: 'trace-fixed', question: 'current question', history: [{ role: 'user', content: 'trusted question' }, { role: 'assistant', content: 'trusted answer' }] }));
    expect(conversations.startTurn).toHaveBeenCalledWith(expect.objectContaining({ conversationId: CONVERSATION_ID, question: 'current question', scopeSpaceIds: [SPACE_ID] }));
    expect(conversations.completeTurn).toHaveBeenCalledWith(REQUEST_ID, expect.objectContaining({ answer: 'server answer', citations: [expect.objectContaining({ documentId: '00000000-0000-4000-8000-000000000013' })] }));
  });

  it('rejects an invalid stream body before starting AI', async () => {
    await request(app.getHttpServer() as Server).post('/chat/stream').set('x-chat-protocol-version', '1')
      .send({ conversationId: CONVERSATION_ID, requestId: REQUEST_ID, selectedSpaceIds: [], message: ' ' }).expect(400);
  ...
  it('rejects a selected space before creating a turn when the user lacks VIEW', async () => {
    ...
    await request(app.getHttpServer() as Server).post('/chat/stream').set('x-chat-protocol-version', '1')
      .send({ conversationId: CONVERSATION_ID, requestId: REQUEST_ID, selectedSpaceIds: [SPACE_ID], message: 'question' }).expect(403);
    expect(conversations.startTurn).not.toHaveBeenCalled();
  });

  it('requires turn ownership before forwarding explicit cancellation', async () => {
    activeRuns.start(REQUEST_ID, USER_ID);
    await request(app.getHttpServer() as Server).post(`/chat/${REQUEST_ID}/cancel`).expect(201).expect({ status: 'cancelling' });
    expect(conversations.getOwnedTurn).toHaveBeenCalledWith(expect.objectContaining({ id: USER_ID }), REQUEST_ID);
